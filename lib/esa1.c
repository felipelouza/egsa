...
int esa_print_suff(int_suff *SA, int_lcp *LCP, size_t size){
...
inline void esa_write(heap *h, heap_node *node, int_lcp lcp) {
...
inline void induce(heap* h, heap_node *node, int_lcp lcp){
		...
		if(h->size_induced_buffer[node->ESA[node->u_idx].c_ant] == INDUCED_SIZE){//induced_buffer
			...
			fwrite(h->induced_buffer[node->ESA[node->u_idx].c_ant], sizeof(t_INDUCED), INDUCED_SIZE, h->fSIGMA[node->ESA[node->u_idx].c_ant]);
		}	
		...
		fwrite(&node->key, sizeof(int_text), 1, h->fSIGMA[node->ESA[node->u_idx].c_ant]);
		fwrite(&lcp, sizeof(int_lcp), 1, h->fSIGMA[node->ESA[node->u_idx].c_ant]);
...
int esa_write_all(int_suff* SA, int_lcp* LCP, t_TEXT *Text, char *c_dir, char *c_file) {
	...
	for(; i < Text->length; i++){
		...
		//bwt
		c_ant = 0;
		if(SA[i] > 0) c_ant = Text->c_buffer[SA[i] - 1];
	...
	fwrite(&Text->length, sizeof(int_suff), 1, f_out);
	i = 0;
	fwrite(&i, sizeof(int_lcp), 1, f_out);
...
int esa_build(t_TEXT *Text, int_text n, int sigma, char *c_dir, char* c_file){
	...
	for(; k < n; k++){
		...
		SA  = (int_suff*) malloc((Text[k].length+1) * sizeof(int_suff));
		if(!SA) perror("esa_build");
		
		LCP = (int_lcp*) malloc((Text[k].length+1) * sizeof(int_lcp));
